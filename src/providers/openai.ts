...
export class OpenAI implements IAI {
  ...
  async ask(prompt: string, options?: AskOptions): Promise<AIResponse> {
    try {
      ...
      throw new AIError(`OpenAI ask failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'openai');
    }
  }
  ...
  async tools(toolDefinitions: ToolDefinition[], request: ToolsRequest): Promise<AIResponse> {
    try {
      ...
      if (error instanceof AIError) throw error;
      throw new AIError(`OpenAI tools failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'openai');
    }
  }
  ...
  async chat(messages: ChatMessage[], options?: ChatOptions): Promise<AIResponse> {
    try {
      ...
      if (error instanceof AIError) throw error;
      throw new AIError(`OpenAI chat failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'openai');
    }
  }
