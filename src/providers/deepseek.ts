...
export class DeepSeek implements IAI {
  ...
  async ask(prompt: string, options?: AskOptions): Promise<AIResponse> {
    try {
      ...
      if (!response.ok) {
        throw new AIError(`DeepSeek API error: ${response.status} ${response.statusText}`, 'deepseek');
      }
      ...
      throw new AIError(`DeepSeek ask failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'deepseek');
    }
  }
  ...
  async tools(toolDefinitions: ToolDefinition[], request: ToolsRequest): Promise<AIResponse> {
    try {
      ...
      if (error instanceof AIError) throw error;
      throw new AIError(`DeepSeek tools failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'deepseek');
    }
  }
  ...
  async chat(messages: ChatMessage[], options?: ChatOptions): Promise<AIResponse> {
    try {
      ...
      if (options?.systemPrompt) {
        deepseekMessages.unshift({ role: 'system', content: options.systemPrompt });
      }
      ...
      if (error instanceof AIError) throw error;
      throw new AIError(`DeepSeek chat failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'deepseek');
    }
  }
