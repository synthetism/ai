...
export class Grok implements IAI {
  ...
  async ask(prompt: string, options?: AskOptions): Promise<AIResponse> {
    try {
      ...
      if (!response.ok) {
        throw new AIError(`Grok API error: ${response.status} ${response.statusText}`, 'grok');
      }
      ...
      throw new AIError(`Grok ask failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'grok');
    }
  }
  ...
  async tools(toolDefinitions: ToolDefinition[], request: ToolsRequest): Promise<AIResponse> {
    try {
      ...
      if (error instanceof AIError) throw error;
      throw new AIError(`Grok tools failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'grok');
    }
  }
  ...
  async chat(messages: ChatMessage[], options?: ChatOptions): Promise<AIResponse> {
    try {
      ...
      if (error instanceof AIError) throw error;
      throw new AIError(`Grok chat failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'grok');
    }
  }
