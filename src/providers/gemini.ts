...
export class Gemini implements IAI {
  ...
  async ask(prompt: string, options?: AskOptions): Promise<AIResponse> {
    try {
      ...
      if (!response.ok) {
        ...
        throw new AIError(`Gemini API error: ${response.status} ${response.statusText} - ${errorText}`, 'gemini');
      }
      ...
      throw new AIError(`Gemini ask failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'gemini');
    }
  }
  ...
  async tools(toolDefinitions: ToolDefinition[], request: ToolsRequest): Promise<AIResponse> {
    try {
      ...
      if (error instanceof AIError) throw error;
      throw new AIError(`Gemini tools failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'gemini');
    }
  }
  ...
  async chat(messages: ChatMessage[], options?: ChatOptions): Promise<AIResponse> {
    try {
      ...
      if (error instanceof AIError) throw error;
      throw new AIError(`Gemini chat failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'gemini');
    }
  }
